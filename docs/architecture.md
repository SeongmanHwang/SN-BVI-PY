...
역점역은 `brf/reverse_translator` (Inspector·내용 비교용). 파서는 **복합 문장부호 최장 일치 → 온표/수표/로마자 상태 → 된소리 → 약자 → 음절 분석** 순이며, 미인식 셀은 `·` 대신 `<U:…>`로 남긴다. 로마자 모드는 공백·괄호를 유지하고, 대문자 약어 뒤 한글 조사에서만 종료한다. **정합의 최종 척도는 ASCII 셀 비교**이지 역점역 유사도가 아니다.

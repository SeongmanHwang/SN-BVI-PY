...
12. **된소리·초성·가류 약자·VC 약자·중성** — 음절 조립  
    ...
    - **종성 ㅍ vs 마침표** (`4`): 짧은 어절(≤2음절, 조립 중 포함) 뒤 공백 없이 → ㅍ;
      긴 어절(≥3) 뒤 → `.`. 점역은 짧은 어절 뒤 `.` 앞에 공백을 넣음.
    - **종성 ㅌ vs 물음표** (`8`): 문장 끝(공백·EOL·밑줄 `,-`/`-'`)에서
      단독 어절 또는 흔한 ㅌ받침 음절(같/끝/밑/밭/맡 …) → ㅌ; 그 외 → `?`.
13. **1칸 구두점** — `PUNCT_SINGLE` (`1`→`,` 등; 마침표 앞 구분 공백은 제거).
    쌍점은 1칸 `3`이 아니라 복합 `"1`(⠐⠂).
...
| `7a7,-…-'` | `ⓐ<u>…</u>` | 인라인 결합도 가능하나, PDF 향찰은 **2행** |
| `7`…`7` | ‘…’ | 드러냄표 (`7a7` 원문자보다 후순위) |
| `"1` / `11` | : | 쌍점(규정 ⠐⠂; `11`은 관례). **`3`(종성 ㄴ) 아님** |

보기 중첩 박스는 PDF에서 표선 묵자 행(`────`)으로 펴고, 향찰 입체 표기는
**밑줄 본문 한 줄 + 원문자 한 줄**로 직렬화한다 (`pdf/graphic_linearize.py`).
